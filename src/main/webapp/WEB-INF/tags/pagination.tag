...
<ul class="pagination pull-right">
	...
	<c:choose>
		...
		<c:otherwise>
			<li><a href="?page=0${searchParamsUrl}${sortParamsUrl}">&laquo;</a></li>
		</c:otherwise>
	</c:choose>
	...
	<c:forEach var="i" begin="${begin}" end="${end}">
		<c:choose>
			...
			<c:otherwise>
				<li><a href="?page=${i-1}${searchParamsUrl}${sortParamsUrl}">${i}</a></li>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<c:choose>
		...
		<c:otherwise>
			<li><a href="?page=${page.totalPages-1}${searchParamsUrl}${sortParamsUrl}">&raquo;</a></li>
		</c:otherwise>
	</c:choose>
