...
$('#deleteConfirmModal_deleteButton').on('click', function () {
	...
	$.ajax({
		type: 'POST',
		url: '/${currentModule}/${currentPage}/delete',
		timeout: 60000,
		data : {
			id : '${item.id}'
		},
		...
		complete: function() {
			...
			$('#deleteConfirmModal').on('hidden.bs.modal', function () {
				if (type == 'MODULE') {
					$(window.location).attr('href', '/${currentModule}/${currentPage}/tree?currentSystem=${item.system}');
				} else {
					$(window.location).attr('href', '/${currentModule}/${currentPage}/tree?currentFunctionId=${item.parent.id}');
				}
			})
		},
