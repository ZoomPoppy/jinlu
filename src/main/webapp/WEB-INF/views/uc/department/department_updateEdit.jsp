		...
		$.ajax({
			...
			complete : function() {
				...
				$('#editModal').on('hidden.bs.modal', function() {
					$(window.location).attr('href', '/${currentModule}/${currentPage}/tree?currentDepartmentId=${item.id}');
				})
			},
					...
					<div class="form-group">
						...
						<select id="categoryId" name="categoryId" class="form-control">
							<c:forEach items="${departmentCategories}" var="departmentCategory">
								<option value="${departmentCategory.id}" ${item.categoryId == departmentCategory.id ? "selected" : ""}>${departmentCategory.name}</option>
							</c:forEach>
						</select>
					</div>
				</div>
