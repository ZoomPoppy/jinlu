...
<script type="text/javascript">
	function zTreeOnClick(event, treeId, treeNode) {
		...
		$.ajax({
			...
			success : function(data, textStatus) {
				if (data.status == 0) {
					var id = data.data.id;
					$('#departmentDetails input[name=name]').val(data.data.name);
					$('#departmentDetails input[name=description]').val(data.data.description);
					$('#departmentDetails #addChildButton').off('click');
					$('#departmentDetails #addChildButton').on('click', function() {
						remoteModal('/${currentModule}/${currentPage}/addChildEdit?parentId=' + id);
					});
					$('#departmentDetails #editButton').off('click');
					$('#departmentDetails #editButton').on('click', function() {
						remoteModal('/${currentModule}/${currentPage}/updateEdit?id=' + id);
					});
					$('#departmentDetails #deleteButton').off('click');
					$('#departmentDetails #deleteButton').on('click', function() {
	...
	$(document).ready(function() {
		...
		if (currentDepartmentId != '') {
			var currentSelectNode = zTreeObj.getNodeByParam("id", currentDepartmentId);
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div id="departmentDetails">
								...
								<div class="form-group">
									<button id="addChildButton" type="button" class="btn btn-primary btn-sm">添加子部门</button>
